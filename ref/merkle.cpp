...
void merkle_sign(uint8_t *sig, unsigned char *root,
                 ...
                 uint32_t idx_leaf, uint32_t *counter_out,PunPRF* PunPRFi,unsigned char cache[KEY_SIZE*LEAF_NUM])
{
#define MAX_HASH_TRIALS_WOTS (1 << (20))
    unsigned char *auth_path = sig + SPX_WOTS_BYTES;
    struct leaf_info_x1 info;
    memset(&info,0,sizeof(struct leaf_info_x1));
    ...
    *counter_out = 0;
    ...
    treehashx1(root, auth_path, ctx,
                ...
                SPX_TREE_HEIGHT,
                wots_gen_leafx1,
                tree_addr, &info,PunPRFi,cache);
}

/* Compute root node of the top-most subtree. */
void merkle_gen_root(unsigned char *root, const spx_ctx *ctx,PunPRF* PunPRFi)
{
    ...
    // PunPRF* PunPRFi=new PunPRF(ctx->sk_seed,SPX_D-1);
    //PunPRF* PunPRFi=new PunPRF(SPX_D-1);
    unsigned char cache[KEY_SIZE*LEAF_NUM]={0};
    merkle_sign(auth_path, root, ctx,
                wots_addr, top_tree_addr,
                ~0 /* ~0 means "don't bother generating an auth path */,
                &counter,PunPRFi,cache);
}
